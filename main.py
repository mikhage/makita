...
# Navigatie tussen pagina's. Streamlit onthoudt automatisch de session_state,
# dus data blijft bewaard bij het wisselen van pagina.
...
# PAGINA: CO₂ Calculator
    ...
    with tab2:
        ...
        if intensity_file:
            ...
            # ✅ FIX: In CO₂ Calculator tab — sla gekozen stroomtype op in session_state
# Voeg na de regel met radio-button dit toe:
k = c3.radio("Stroomtype", ['Groen', 'Grijs'], horizontal=True, key=sheet + een)
st.session_state[f"keuze_{sheet}_{een}"] = k
# st.session_state[f"keuze_{sheet}_{een}"] = k

# ✅ FIX: In Visualisatie tab — haal keuze op uit session_state
# Zoek naar deze regel:
keuze_key = f"keuze_{sheet}_{eenheid}"
stroomtype = st.session_state.get(keuze_key, "grijs").lower()
factor = st.session_state.emission_factors[eenheid][stroomtype]